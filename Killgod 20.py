...
def consume_iframe_action():
    ...
    if not st.session_state.get("_kg_query_lock"):
        ...
        if raw.startswith("attack"):
            ...
            if target_id:
                apply_damage_to_target_id(target_id, 1, "Dark Strike")
            else:
                apply_damage_to_nearest(1, "Dark Strike")
        elif raw.startswith("magic"):
            target_id = raw.split(":", 1)[1] if ":" in raw else None
            if target_id:
                apply_damage_to_target_id(target_id, 2, "Hellfire Blast")
            else:
...
with col1:
    ...
    <script type="module">
        ...
        document.getElementById('btn-attack').addEventListener('click', () => {{
            ...
            // Hide a lethal target immediately in the view, then commit the
            // authoritative action. The backend remains the source of truth.
