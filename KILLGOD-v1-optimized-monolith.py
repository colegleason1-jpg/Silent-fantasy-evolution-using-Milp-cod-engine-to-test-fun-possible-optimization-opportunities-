...
def is_hostile(creature):
    return creature.get('type') in ENEMY_TYPES

def living_creatures(creatures):
    return [c for c in creatures if c.get('alive', True) and c.get('is_alive', True)]

def living_hostiles(creatures):
    return [c for c in living_creatures(creatures) if is_hostile(c)]

def sync_enemy_indexes():
    creatures = st.session_state.get('creatures', [])
    enemy_pool = []
    next_id = 1
    for creature in creatures:
        creature.setdefault('alive', True)
        creature.setdefault('is_alive', creature.get('alive', True))
        if is_hostile(creature):
            creature['id'] = creature.get('id', next_id)
            next_id = max(next_id, creature['id'] + 1)
            enemy_pool.append(creature)
    st.session_state.enemies = enemy_pool

def kill_enemy(enemy_id):
    for enemy in st.session_state.get('enemies', []):
        if enemy.get('id') == enemy_id:
            enemy['is_alive'] = False
            enemy['alive'] = False
            enemy['hp'] = 0
            break
    st.session_state.quest_progress = st.session_state.get('quest_progress', 0) + 1

def nearest_hostile():
    hostiles = living_hostiles(st.session_state.get('creatures', []))
    if not hostiles:
        return None, None
...
def advance_player(dx=0, dz=0, label='Advance'):
    ...
    st.session_state.player_position = st.session_state.get('player_position', 0) + abs(dx) + abs(dz)
    process_turn_advance(label)

def strike_nearest():
    target, dist = nearest_hostile()
    if not target:
        st.session_state.message = 'No living enemies remain in this region.'
        return
    if dist > 14:
        st.session_state.message = f"{target['name']} is too far away. Move closer to strike."
        process_turn_advance('Missed strike attempt')
        return
    target['hp'] -= 1
    if target['hp'] <= 0:
        st.session_state.kills += 1
        kill_enemy(target['id'])
        st.session_state.message = f"Permanently vanquished {target['name']}! Soul harvested."
    else:
        st.session_state.message = f"Damaged {target['name']}! ({target['hp']}/{target['max_hp']} HP left)"
    process_turn_advance('Dark Strike')

def cast_hellfire():
    hostiles = living_hostiles(st.session_state.get('creatures', []))
    if not hostiles:
        st.session_state.message = 'Hellfire found no valid targets.'
        return
    targets_hit = 0
    kills = 0
    for target in hostiles:
        ...
        if dist <= 18:
            ...
            if target['hp'] <= 0:
                ...
                kill_enemy(target['id'])
    if targets_hit == 0:
        st.session_state.message = 'Hellfire dissipated before reaching any enemy.'
    else:
        st.session_state.message = f'Hellfire scorched {targets_hit} enemies and claimed {kills} souls.'
    process_turn_advance('Hellfire Blast', gold_delta=5)

def dark_ritual():
    if st.session_state.gold < 20:
        st.session_state.message = 'Not enough gold for a dark ritual.'
        return
    process_turn_advance('Dark Ritual', health_delta=20, gold_delta=-20)
    st.session_state.message = 'Dark ritual completed. Flesh renewed by abyssal power.'
...
with col1:
    ...
    active_creatures_for_render = living_creatures(st.session_state.creatures)
    creatures_json = json.dumps([{
        'id': c.get('id', 0), 'name': c['name'], 'x': c['x'], 'z': c['z'], 'height': c.get('height', 0), 
        'alive': c['alive'], 'type': c['type'], 'hp': c['hp'], 'max_hp': c['max_hp'], 'element': c.get('element', 'shadow')
    ...
    <div style="background: #05050a; border: 1px solid #3a1a1a; border-radius: 6px; padding: 4px; text-align: center; position: relative;">
        <div id="canvas-container" style="width: 100%; height: 420px; border-radius: 4px; overflow: hidden; box-shadow: inset 0 0 20px rgba(0,0,0,0.9); position: relative;">
    ...
    <script type="module">
        ...
        function animate() {{
            ...
            for (const obj of creatureObjects) {
                const meta = obj.userData.meta || {};
                const sway = elapsed * (meta.type === 'boss' ? 1.4 : meta.type === 'npc' ? 2.0 : 2.8) + (meta.x || 0) * 0.12;
                obj.position.y = (obj.userData.baseY || obj.position.y) + Math.sin(sway) * (meta.type === 'boss' ? 0.22 : 0.14);
                obj.rotation.y += meta.type === 'npc' ? 0.0025 : 0.0015;
                if (obj.userData.weapon) obj.userData.weapon.rotation.z = (meta.type === 'npc' ? -0.45 : 0.25) + Math.sin(sway * 1.7) * 0.14;
                if (obj.userData.aura) { obj.userData.aura.material.opacity = 0.24 + (Math.sin(sway * 2.1) + 1) * 0.09; obj.userData.aura.rotation.z += 0.01; }
                if (obj.userData.halo) obj.userData.halo.rotation.z += 0.03;
                if (obj.userData.wings) { obj.userData.wings[0].rotation.z = 0.18 + Math.sin(sway * 4.2) * 0.22; obj.userData.wings[1].rotation.z = -0.18 - Math.sin(sway * 4.2) * 0.22; }
                if (obj.userData.cape) obj.userData.cape.rotation.x = Math.sin(sway * 2.5) * 0.08;
            }
    ...
    components.html(three_html, height=455, scrolling=False)
    ...
    with ac1:
        if st.button("⚔️ Strike (Nearest)", use_container_width=True):
            alive_enemies = [c for c in st.session_state.creatures if c.get('alive', True) and c['type'] in ['enemy', 'boss']]
            if alive_enemies:
                ...
                if dist <= 14:
                    ...
                    if target['hp'] <= 0:
                        target['alive'] = False
                        st.session_state.kills += 1
                        msg = f"Permanently vanquished {target['name']}! Soul harvested."
                    else:
                        msg = f"Damaged {target['name']}! ({target['hp']}/{target['max_hp']} HP left)"
