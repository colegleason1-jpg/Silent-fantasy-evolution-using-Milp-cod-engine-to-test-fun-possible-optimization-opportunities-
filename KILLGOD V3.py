...
def creature_key(creature):
    return f"{creature.get('type', '')}|{creature.get('name', '')}|{creature.get('x', '')}|{creature.get('z', '')}"
...
def remove_creature_from_world(target):
    ...
    st.session_state.defeated_enemy_keys.add(creature_key(target))
    st.session_state.creatures = [
        c for c in st.session_state.get('creatures', [])
        if not (
            c.get('name') == target.get('name') and
            c.get('x') == target.get('x') and
            c.get('z') == target.get('z')
        )
    ]
    if 'world_data' in st.session_state:
        st.session_state.world_data['creatures'] = st.session_state.creatures
            ...
            if live_enemies:
                ...
                if dist <= 14:
                    ...
                    if target['hp'] <= 0:
                        ...
                        remove_creature_from_world(target)
