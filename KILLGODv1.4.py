...
def live_creatures():
    assign_ids_once(st.session_state.get('creatures', []))
    st.session_state.creatures = [
        c for c in st.session_state.get('creatures', [])
        if (not is_hostile(c)) or c.get('hp', 0) > 0
    ]
...
def live_hostiles():
    return [c for c in live_creatures() if is_hostile(c)]

def nearest_hostile():
    enemies = live_hostiles()
    if not enemies:
        return None, None
...
def delete_enemy_forever(enemy_id):
...
def strike_nearest():
    ...
    if not target:
        st.session_state.message = 'No holy entities remaining nearby.'
        return
    if dist > 12:
    ...
    if target['hp'] <= 0:
        ...
        delete_enemy_forever(target['id'])
        st.session_state.kills += 1
        st.session_state.message = f'{slain_name} was annihilated and removed from the world.'
        process_turn_advance(st.session_state.message, gold_delta=25, faction_updates={'Heavenly Host': -5})
    else:
...
def cast_hellfire():
    ...
    for enemy_id in slain:
        delete_enemy_forever(enemy_id)
        st.session_state.kills += 1
    if hits == 0:
        process_turn_advance('Hellfire dissipated before reaching any enemy.', gold_delta=0)
    else:
        process_turn_advance(f'Hellfire scorched {hits} enemies and destroyed {len(slain)} of them.', gold_delta=10, faction_updates={'Heavenly Host': -8})
...
with col1:
    ...
    active_creatures_for_render = live_creatures()
    ...
    with mc1:
        if st.button("Move / Advance", use_container_width=True):
            move_player(dx=2, dz=0, label="Advance Through Wastes")
            st.rerun()
    with mc2:
        if st.button("⬆️ North", use_container_width=True):
            move_player(dz=-2, label="Crusade Movement (North)")
            st.rerun()
    with mc3:
        if st.button("⬅️ West", use_container_width=True):
            move_player(dx=-2, label="Crusade Movement (West)")
            st.rerun()
    with mc4:
        if st.button("➡️ East", use_container_width=True):
            move_player(dx=2, label="Crusade Movement (East)")
            st.rerun()
    with mc5:
        if st.button("⬇️ South", use_container_width=True):
            move_player(dz=2, label="Crusade Movement (South)")
            st.rerun()

    ac1, ac2, ac3 = st.columns(3)
    with ac1:
        if st.button("⚔️ Dark Strike (Nearest)", use_container_width=True):
            strike_nearest()
            st.rerun()
    with ac2:
    ...
    with ac3:
        if st.button("🗣️ Consult Oracle", use_container_width=True):
            npcs = [c for c in live_creatures() if c['type'] == 'npc']
            if npcs:
