...
# GameState contract: session_state is the only authority.
# The Three.js iframe is a view. It may animate, but it may not invent HP, death, or quests.
...
def creature_key(creature):
    ...
    if cid:
        return str(cid)
    stamped = "ent-{0}-{1}-{2}-{3}".format(
        creature.get("type", "unk"),
        str(creature.get("name", "na")).replace(" ", "_"),
        creature.get("x", 0),
        creature.get("z", 0),
    )
    creature["id"] = stamped
...
def ensure_entity_ids(creatures):
    used = set()
    for i, c in enumerate(creatures):
        cid = creature_key(c)
        if cid in used:
            cid = "{0}-dup{1}".format(cid, i)
            c["id"] = cid
        used.add(cid)
        c.setdefault("alive", True)
        c.setdefault("max_hp", c.get("hp", 1))
        c.setdefault("dead_permanent", False)
    return creatures


def dead_ids():
    ids = st.session_state.get("dead_ids", [])
    if not isinstance(ids, list):
        ids = list(ids)
        st.session_state.dead_ids = ids
    return ids


def quest_flags():
    flags = st.session_state.get("quest_flags", [])
    if not isinstance(flags, list):
        flags = list(flags)
        st.session_state.quest_flags = flags
    return flags
...
def is_dead(cid):
    return cid in dead_ids()


def commit_death(target):
    # Single authoritative death operation. The entity is removed from both
    # state rosters so a Streamlit rerun cannot bring it back.
    cid = creature_key(target)
    target["hp"] = 0
    target["alive"] = False
    target["dead_permanent"] = True
    ids = dead_ids()
    if cid not in ids:
        ids.append(cid)
    st.session_state.dead_ids = ids
    add_flag("kill:{0}".format(cid))
    if cid.startswith("starter-"):
        add_flag("q01_starter_dead:{0}".format(cid))
    if cid == "boss-jesus-protector":
        add_flag("q99_son_fallen")
    roster = st.session_state.get("creatures", [])
    remaining = [c for c in roster if creature_key(c) != cid]
    st.session_state.creatures = remaining
    if "world_data" in st.session_state:
        st.session_state.world_data["creatures"] = remaining
    return cid


def get_roster():
    roster = st.session_state.get("creatures", [])
    ensure_entity_ids(roster)
    st.session_state.creatures = roster
    if "world_data" in st.session_state:
        st.session_state.world_data["creatures"] = roster
    return roster


def visible_in_world(c):
    ctype = c.get("type")
    cid = creature_key(c)
    if ctype == "npc":
        return True
    if is_dead(cid) or c.get("dead_permanent") or c.get("hp", 0) <= 0 or c.get("alive") is False:
        return False
    if ctype == "boss" and cid == "boss-jesus-protector":
        return st.session_state.get("quest_stage", 1) >= 3
    return True


def get_live_creatures():
    return [c for c in get_roster() if visible_in_world(c)]


def nearest_hostile():
    px = st.session_state.player_x
    pz = st.session_state.player_z
    live = [c for c in get_live_creatures() if c.get("type") in ("enemy", "boss")]
    if not live:
        return None
    live.sort(key=lambda c: math.hypot(c["x"] - px, c["z"] - pz))
    return live[0]


def apply_damage_to_target_id(target_id, amount, action_name):
    target = next((c for c in get_live_creatures() if creature_key(c) == str(target_id) and c.get("type") in ("enemy", "boss")), None)
    if target is None:
        process_turn_advance("That holy entity is already gone.", gold_delta=0)
        return None
    ...
    if dist > 14:
        ...
        return None
    target["hp"] = max(0, int(target.get("hp", 0)) - int(amount))
    st.session_state.last_fx = {"id": creature_key(target), "name": target.get("name"), "killed": target["hp"] <= 0, "hp": target["hp"], "max_hp": target.get("max_hp", 1)}
    if target["hp"] <= 0:
        slain = target.get("name", "enemy")
        commit_death(target)
        st.session_state.kills = int(st.session_state.get("kills", 0)) + 1
        process_turn_advance("Permanently vanquished {0}! Soul harvested.".format(slain), gold_delta=25, faction_updates={"Heavenly Host": -5})
    else:
        process_turn_advance("Damaged {0}! ({1}/{2} HP left)".format(target.get("name"), target["hp"], target.get("max_hp", 1)), gold_delta=10, faction_updates={"Heavenly Host": -2})
    return target


def apply_damage_to_nearest(amount, action_name):
    ...
    if dist > 14:
        ...
        return None
    target["hp"] = max(0, int(target.get("hp", 0)) - int(amount))
    st.session_state.last_fx = {
        "id": creature_key(target),
    ...
    if target["hp"] <= 0:
        ...
        commit_death(target)
        st.session_state.kills = int(st.session_state.get("kills", 0)) + 1
        process_turn_advance(
            "Permanently vanquished {0}! Soul harvested.".format(slain),
            gold_delta=25,
            faction_updates={"Heavenly Host": -5},
        )
    else:
        process_turn_advance(
            "Damaged {0}! ({1}/{2} HP left)".format(
                target.get("name"), target["hp"], target.get("max_hp", 1)
            ),
            gold_delta=10,
            faction_updates={"Heavenly Host": -2},
        )
    return target
        ...
        if node_id in species_map:    
            ...
            for i in range(count):    
                if suitable:    
                    ...
                    creatures.append({
                        'id': 'mob-{0}-{1}-{2}-{3}'.format(node_id, i, loc['x'], loc['z']),
...
def init_game_state():
    if 'initialized' not in st.session_state:
        ...
        st.session_state.dead_ids = []
        st.session_state.quest_flags = []
        ...
        st.session_state.creatures = ensure_entity_ids(world['creatures'])

        st.session_state.active_quest = "Chapter 1: Slay 3 holy zealots or sentinels near spawn."
        st.session_state.quest_target_kills = 3


def starter_kill_count():
    return sum(1 for flag in quest_flags() if flag.startswith("q01_starter_dead:"))


def check_quest_progress():
    stage = st.session_state.get("quest_stage", 1)
    # Ensure any new spawn kills or general kills advance quest stages reliably
    total_kills = int(st.session_state.get("kills", 0))
    starters_dead = starter_kill_count()
    
    if stage == 1 and (starters_dead >= 3 or total_kills >= 3):
    ...
    elif stage == 2 and total_kills >= 7:
    ...
    elif stage >= 3 and "q99_son_fallen" in quest_flags() and stage < 4:
        st.session_state.quest_stage = 4
        add_flag("quest:4")
        st.session_state.active_quest = "Chapter 4: The Throne stands unguarded. God remains."
        st.session_state.message = "The Protector has fallen. The Throne is next."


init_game_state()

def consume_iframe_action():
    raw = None
    try:
        raw = st.query_params.get("kg_action")
    except Exception:
        raw = None
    if isinstance(raw, list):
        raw = raw[0] if raw else None
    if not raw:
        st.session_state._kg_query_lock = False
        return
    if not st.session_state.get("_kg_query_lock"):
        st.session_state._kg_query_lock = True
        if raw.startswith("attack"):
            target_id = raw.split(":", 1)[1] if ":" in raw else None
            if target_id:
                apply_damage_to_target_id(target_id, 2, "Dark Strike")
            else:
                apply_damage_to_nearest(2, "Dark Strike")
        elif raw.startswith("magic"):
            target_id = raw.split(":", 1)[1] if ":" in raw else None
            if target_id:
                apply_damage_to_target_id(target_id, 5, "Hellfire Blast")
            else:
                apply_damage_to_nearest(5, "Hellfire Blast")
        elif raw == "rest":
            if st.session_state.gold >= 20:
                process_turn_advance("Dark Ritual completed.", health_delta=25, gold_delta=-20)
            else:
                process_turn_advance("Not enough gold for a Dark Ritual.", gold_delta=0)
    try:
...
def process_turn_advance(action_name: str, health_delta=0, gold_delta=0, faction_updates=None):
    ...
    st.session_state.turn_log.append("Turn {0}: {1}".format(st.session_state.turn - 1, action_name))
    st.session_state.message = action_name
    if "world_data" in st.session_state:
        st.session_state.world_data["creatures"] = st.session_state.creatures
    check_quest_progress()


consume_iframe_action()
...
with col1:
    ...
    creatures_json = json.dumps([{
        ...
        'alive': True, 'type': c['type'], 'hp': c['hp'], 'max_hp': c['max_hp']
    } for c in active_creatures_for_render])
    last_fx_json = json.dumps(st.session_state.get('last_fx') or {})
    ...
    <script type="module">
        ...
        function commitActionToGameState(action) {{
            const parentHref = window.parent.location.href.split('#')[0];
            const url = new URL(parentHref);
            url.searchParams.set('kg_action', action);
        ...
        document.getElementById('btn-attack').addEventListener('click', () => {{
            ...
            if (!hasValidTarget) {{
                commitActionToGameState('attack');
                return;
            }}
            if (targetIsKilled && targetEnemyGroup) {{
                targetEnemyGroup.visible = false;
                if (targetEnemyLabel) targetEnemyLabel.style.display = 'none';
            }}
            commitActionToGameState('attack:' + targetEnemyId);
        }});

        document.getElementById('btn-magic').addEventListener('click', () => {{
            if (isAnimating) return;
            if (!hasValidTarget) {{
                commitActionToGameState('magic');
                return;
            }}
            if (targetIsKilled && targetEnemyGroup) {{
                targetEnemyGroup.visible = false;
            ...
            commitActionToGameState('magic:' + targetEnemyId);
            ...
            if (isAnimating) {{
                if (animType === 'attack') {{
                    ...
                    if (dieStep >= Math.PI) {{
                        isAnimating = false;
                        if (pendingCommit) {{
                            const act = pendingCommit;
                            pendingCommit = null;
                            commitActionToGameState(act);
                        }}
                    }}
    ...
    with ac1:
        if st.button("⚔️ Strike (Nearest)", use_container_width=True):
            apply_damage_to_nearest(1, "Dark Strike")
            st.rerun()
    with ac2:
        if st.button("🔮 Hellfire Blast", use_container_width=True):
            apply_damage_to_nearest(2, "Hellfire Blast")
            st.rerun()
    with ac3:
        if st.button("🗣️ Consult Devil", use_container_width=True):
            npcs = [c for c in get_live_creatures() if c.get('type') == 'npc']
            if npcs:
                ...
                if dist <= 12:
                    lines = npc.get('dialogues') or ["The compact holds. Keep harvesting."]
                    line = lines[st.session_state.turn % len(lines)]
                    add_flag("met_devil")
                    process_turn_advance("Devil: " + line)
                else:
                    process_turn_advance("The Devil is too far. Close the distance.")
            else:
                process_turn_advance("The Devil is not on this plane.")
            st.rerun()
...
with col2:
    ...
    if stage == 1:
        ...
        st.write("Objective: slay the 3 holy sentinels around spawn.")
        st.write("Progress: {0} / 3 starter kills".format(starter_kill_count()))
        st.progress(min(1.0, starter_kill_count() / 3.0))
    elif stage == 2:
        st.markdown("**Chapter 2: Mountain of Prophets**")
        st.write("Objective: harvest 7 souls total (keep the dead dead).")
        st.write("Progress: {0} / 7 kills".format(st.session_state.kills))
        st.progress(min(1.0, st.session_state.kills / 7.0))
    elif stage == 3:
        st.markdown("**Chapter 3: The Protector**")
        st.write("Objective: defeat Jesus to breach the Throne.")
        jesus_dead = "q99_son_fallen" in quest_flags()
        st.write("Protector is fallen." if jesus_dead else "Protector still stands.")
        st.progress(1.0 if jesus_dead else 0.7)
    else:
        st.markdown("**Chapter 4: The Throne**")
        st.write("The Protector is gone. God remains.")
        st.progress(1.0)
    ...
    active_targets = [c for c in get_live_creatures() if c.get('type') in ['enemy', 'boss']]
    ...
    st.caption("Dead IDs this run: {0}".format(len(dead_ids())))
    if st.button("🔄 Restart Crusade (New Game)", use_container_width=True):
