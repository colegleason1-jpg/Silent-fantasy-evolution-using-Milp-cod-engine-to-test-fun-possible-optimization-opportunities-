...
<style>
    ...
    /* Tighter padding to completely eliminate scrolling */
...
def init_game_state():
    if 'initialized' not in st.session_state:
        ...
        st.session_state.attack_nonce = 0
        st.session_state.magic_nonce = 0
...
def execute_attack():
    ...
    for c in world['creatures']:
        # Strict melee range check (within 3 grid units)
        if c['alive'] and abs(c['x'] - st.session_state.player_x) <= 3 and abs(c['z'] - st.session_state.player_z) <= 3:
            hit_target = c
            break
    ...
    if hit_target:
        ...
        if hit_target['hp'] <= 0:
            ...
            st.session_state.message = f"⚔️ Vanquished {hit_target['name']} with a brutal melee lunge and blood splatter! Looted 40 Gold."
            process_turn_advance(f"Defeated {hit_target['name']}", gold_delta=40, faction_updates={'Dragons': -8})
        else:
            st.session_state.message = f"⚔️ Melee lunge struck {hit_target['name']} for {dmg} damage with blood burst! ({hit_target['hp']} HP left)"
            process_turn_advance(f"Attacked {hit_target['name']}")
    else:
        st.session_state.message = "⚠️ Melee range miss! Move closer to the enemy to lunge and strike."

def execute_magic():
    world = st.session_state.world_data
    # Long range fireball target check (within 8 grid units)
    magic_target = None
    for c in world['creatures']:
        if c['alive'] and abs(c['x'] - st.session_state.player_x) <= 8 and abs(c['z'] - st.session_state.player_z) <= 8:
            magic_target = c
            break

    st.session_state.magic_nonce += 1

    if st.session_state.gold >= 10:
        if magic_target:
            dmg = random.randint(40, 65)
            magic_target['hp'] -= dmg
            if magic_target['hp'] <= 0:
                magic_target['alive'] = False
                st.session_state.kills += 1
                st.session_state.message = f"🔥 Fireball projectile incinerated {magic_target['name']} from afar! Looted 50 Gold."
                process_turn_advance("Cast Fireball (Kill)", gold_delta=50, faction_updates={'Fairies': 10})
            else:
                st.session_state.message = f"🔥 Fireball projectile slammed into {magic_target['name']} for {dmg} ranged damage! ({magic_target['hp']} HP left)"
                process_turn_advance("Cast Fireball", gold_delta=-10, faction_updates={'Fairies': 10})
        else:
            st.session_state.message = "✨ Cast Fireball into the distance, but no long-range targets were in sight!"
            process_turn_advance("Cast Spell", gold_delta=-10, faction_updates={'Fairies': 10})
    else:
        st.session_state.message = "⚠️ Not enough gold/mana essence to cast fireball!"
...
st.markdown("### 🐉 Dragon Realm: 3D Tactical Adventure")
...
with col1:
    ...
    magic_nonce = st.session_state.magic_nonce

    three_html = f"""
    <div style="background: #05050a; border: 1px solid #2a2a40; border-radius: 6px; padding: 4px; text-align: center;">
        <div id="canvas-container" style="width: 100%; height: 230px; border-radius: 4px; overflow: hidden; box-shadow: inset 0 0 20px rgba(0,0,0,0.9);"></div>
        <p style="color: #00ffcc; font-size: 10px; margin: 3px 0 0 0; font-family: monospace;">
            POS X:{st.session_state.player_x} | Z:{st.session_state.player_z} | KILLS: {st.session_state.kills}
        </p>
    </div>
    <script type="module">
        ...
        const magicNonce = {magic_nonce};
        
        const lastAttackNonce = sessionStorage.getItem('last_attack_nonce') || '0';
        const doAttackAnim = attackNonce > parseInt(lastAttackNonce);
        if (doAttackAnim) {{
            sessionStorage.setItem('last_attack_nonce', attackNonce.toString());
        }}

        const lastMagicNonce = sessionStorage.getItem('last_magic_nonce') || '0';
        const doMagicAnim = magicNonce > parseInt(lastMagicNonce);
        if (doMagicAnim) {{
            sessionStorage.setItem('last_magic_nonce', magicNonce.toString());
        }}

        let bloodParticles = null;
        let fireballMesh = null;
        let fireballTargetX = pX, fireballTargetZ = pZ - 3;
        ...
        if (doAttackAnim) {{
            let targetX = pX, targetZ = pZ - 1.2;
            creaturesData.forEach(c => {{
                if (c.alive && c.type === 'enemy') {{
                    const cx = (c.x - 24) * 0.7;
                    const cz = (c.z - 24) * 0.7;
                    if (Math.abs(cx - pX) <= 2.5 && Math.abs(cz - pZ) <= 2.5) {{
                        targetX = cx;
                        targetZ = cz;
                    }}
                }}
            ...
            const bloodMat = new THREE.PointsMaterial({{ color: 0xff0000, size: 0.22, transparent: true, opacity: 1.0 }});
            bloodParticles = new THREE.Points(bloodGeo, bloodMat);
            scene.add(bloodParticles);
            window.bloodVelocities = velocities;
            window.attackTargetZ = targetZ;
        }}
        ...
        if (doMagicAnim) {{
            creaturesData.forEach(c => {{
                if (c.alive && c.type === 'enemy') {{
                    const cx = (c.x - 24) * 0.7;
                    const cz = (c.z - 24) * 0.7;
                    if (Math.abs(cx - pX) <= 6 && Math.abs(cz - pZ) <= 6) {{
                        fireballTargetX = cx;
                        fireballTargetZ = cz;
                    }}
                }}
            }});
            const fbGeo = new THREE.SphereGeometry(0.35, 8, 8);
            const fbMat = new THREE.MeshBasicMaterial({{ color: 0xffaa00 }});
        ...
        function animate() {{
            ...
            if (doAttackAnim) {{
                animStep += 0.1;
                if (animStep < Math.PI) {{
                    playerMesh.position.z = pZ - Math.sin(animStep) * 1.2;
                }}
            }} else {{
            ...
            if (doMagicAnim && fireballMesh) {{
                magicStep += 0.08;
                fireballMesh.position.x = pX + (fireballTargetX - pX) * magicStep;
                fireballMesh.position.z = pZ + (fireballTargetZ - pZ) * magicStep;
                if (magicStep >= 1.0) {{
            ...
            if (bloodParticles) {{
                ...
                bloodParticles.material.opacity -= 0.02;
            }}
    ...
    """
    components.html(three_html, height=270)

    # Action Controls & Movement combined compactly below canvas
    ac1, ac2, ac3 = st.columns(3)
    with ac1:
        if st.button("⚔️ Attack (Melee)", use_container_width=True):
            execute_attack()
            st.rerun()
    with ac2:
        if st.button("🔥 Fireball (Magic)", use_container_width=True):
            execute_magic()
            st.rerun()
    with ac3:
        if st.button("⛺ Rest (15G)", use_container_width=True):
            execute_rest()
            st.rerun()
