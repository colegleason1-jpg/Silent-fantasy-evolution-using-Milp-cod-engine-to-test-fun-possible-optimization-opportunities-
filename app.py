...
def init_game_state():
    if 'initialized' not in st.session_state:
        ...
        st.session_state.message = "Immersive 3D Fantasy Realm loaded successfully with hybrid models!"
...
with col1:
    ...
    # Enhanced Three.js 3D Viewport with Hybrid GLTF and Procedural Composite Models
    three_html = f"""
    ...
    <script type="module">
        ...
        // Hybrid Entity Rendering: GLTF Loaders + Procedural Composite Models
        ...
        creaturesData.forEach(c => {{
            ...
            if (c.name === 'Dragon') {{
                // Load high-detail external model for the Dragon boss with safe procedural fallback
                gltfLoader.load('https://raw.githubusercontent.com/KhronosGroup/glTF-Sample-Models/master/2.0/DragonAttenuation/glTF/DragonAttenuation.gltf', (gltf) => {{
                    const dragonModel = gltf.scene;
                    dragonModel.position.set(cx, cy, cz);
                    dragonModel.scale.set(0.4, 0.4, 0.4);
                    scene.add(dragonModel);
                }}, undefined, (error) => {{
                    createProceduralEntity(cx, cy, cz, 0xff5500, 'dragon');
                }});
            }} else {{
                // Use clean procedural low-poly composite models for humanoids, beasts, and NPCs
                const color = c.type === 'npc' ? 0xffd700 : 0xff3355;
                createProceduralEntity(cx, cy, cz, color, c.name);
            }}
        }});

        function createProceduralEntity(x, y, z, colorHex, type) {{
            const group = new THREE.Group();
            const mat = new THREE.MeshStandardMaterial({{ color: colorHex, roughness: 0.4 }});
            
            // Detailed low-poly humanoid/beast composite construction
            const bodyGeo = new THREE.CylinderGeometry(0.25, 0.35, 0.7, 5);
            ...
            if (type === 'Elf' || type === 'Devil' || type === 'Goblin') {{
                // Add limbs for humanoid figures
