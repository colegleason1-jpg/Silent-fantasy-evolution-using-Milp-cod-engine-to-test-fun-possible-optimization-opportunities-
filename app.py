...
def execute_attack():
    ...
    if hit_target:
        ...
        if hit_target['hp'] <= 0:
            ...
            st.session_state.message = f"⚔️ Vanquished {hit_target['name']}! Looted 40 Gold."
            process_turn_advance(f"Defeated {hit_target['name']}", gold_delta=40, faction_updates={'Dragons': -8})
        else:
            st.session_state.message = f"⚔️ Struck {hit_target['name']} for {dmg} damage! ({hit_target['hp']} HP left)"
            process_turn_advance(f"Attacked {hit_target['name']}")
    else:
        st.session_state.message = "⚠️ No targets within weapon range! Move closer to enemies."

def execute_magic():
    world = st.session_state.world_data
    if st.session_state.gold >= 10:
        st.session_state.message = "✨ Cast Arcane Nova! Damaged all nearby entities and revealed map secrets."
        for c in world['creatures']:
...
with col1:
    ...
    # Reliable High-Detail Procedural Composite 3D Models (No external network failures)
    three_html = f"""
    ...
    <script type="module">
        ...
        // Generate Terrain Mesh from Python Data
        ...
        // Map python player coordinates to 3D world space
        ...
        // High-Detail Procedural Composite Character Generator
        ...
        creaturesData.forEach(c => {{
            ...
            if (c.name === 'Dragon') {{
                // Epic Multi-Part Dragon Boss Construction
                const bodyMat = new THREE.MeshStandardMaterial({{ color: 0xff4500, roughness: 0.3, metalness: 0.4 }});
                const wingMat = new THREE.MeshStandardMaterial({{ color: 0x8b0000, roughness: 0.5, side: THREE.DoubleSide }});
                ...
                // Wings
                const leftWing = new THREE.Mesh(new THREE.BufferGeometry(), wingMat); // Simplified wing plane
                const wingGeo = new THREE.PlaneGeometry(1.2, 0.6);
                const wingLeft = new THREE.Mesh(wingGeo, wingMat);
                wingLeft.position.set(0.7, 0.4, 0);
                wingLeft.rotation.y = -Math.PI / 4;
                group.add(wingLeft);

                const wingRight = new THREE.Mesh(wingGeo, wingMat);
                wingRight.position.set(-0.7, 0.4, 0);
                wingRight.rotation.y = Math.PI / 4;
                group.add(wingRight);

            }} else if (c.name === 'Elf' || c.name === 'Devil') {{
                // Humanoid / NPC Composite Model
                const skinMat = new THREE.MeshStandardMaterial({{ color: c.name === 'Devil' ? 0x8b008b : 0xffdab9, roughness: 0.5 }});
                const clothMat = new THREE.MeshStandardMaterial({{ color: c.name === 'Devil' ? 0x222222 : 0x2e8b57, roughness: 0.4 }});

                // Torso
                const torso = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.3, 0.7, 6), clothMat);
                ...
                // Arms
                const arms = new THREE.Mesh(new THREE.BoxGeometry(0.7, 0.12, 0.12), clothMat);
                arms.position.y = 0.5;
                group.add(arms);

            }} else {{
                // Beasts & Goblins (Wolf, Bear, Goblin, Fairy)
                const beastColor = c.name === 'Wolf' ? 0x808080 : (c.name === 'Bear' ? 0x5c4033 : (c.name === 'Goblin' ? 0x556b2f : 0xda70d6));
                ...
                const head = new THREE.Mesh(new THREE.SphereGeometry(0.2, 6, 6), beastMat);
        ...
        const targetYPos = pY + 4;
        camera.position.set(pX, targetYPos, pZ + 7);
        ...
        const particleMat = new THREE.PointsMaterial({{ color: 0xaa44ff, size: 0.35 }});
