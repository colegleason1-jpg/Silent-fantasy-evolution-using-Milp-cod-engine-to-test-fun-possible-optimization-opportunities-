...
# Check if there's a deferred action coming from JavaScript animation finish
if st.session_state.pending_action == "attack":
    st.session_state.pending_action = None
    hit_target = None
    for c in st.session_state.world_data['creatures']:
...
elif st.session_state.pending_action == "magic":
    st.session_state.pending_action = None
    magic_target = None
    for c in st.session_state.world_data['creatures']:
    ...
    if st.session_state.gold >= 10:
        if magic_target:
            ...
            if magic_target['hp'] <= 0:
                ...
                st.session_state.kills += 1
                process_turn_advance("Cast Fireball (Kill)", gold_delta=40, faction_updates={'Fairies': 10})
            else:
...
with col1:
    ...
    <div style="background: #05050a; border: 1px solid #2a2a40; border-radius: 6px; padding: 4px; text-align: center;">
        ...
        <div style="margin-top: 8px; display: flex; justify-content: center; gap: 8px;">
            <button id="btn-attack" style="background:#cc2222; color:#fff; border:none; padding:6px 12px; border-radius:4px; cursor:pointer; font-weight:bold; font-size:11px;">⚔️ Attack (Melee)</button>
            <button id="btn-magic" style="background:#ff8800; color:#fff; border:none; padding:6px 12px; border-radius:4px; cursor:pointer; font-weight:bold; font-size:11px;">🔥 Fireball (Magic)</button>
    ...
    <script type="module">
        ...
        function triggerBloodSplatter(x, y, z) {{
            ...
            const bloodMat = new THREE.PointsMaterial({{ color: 0xff0000, size: 0.3, transparent: true, opacity: 1.0 }});
        ...
        function animate() {{
            ...
            if (isAnimating) {{
                if (animType === 'attack') {{
                    ...
                    if (animStep <= Math.PI * 2) {{
                        playerMesh.position.z = pZ - Math.sin(animStep) * 1.8;
                    }} else {{
                        playerMesh.position.z = pZ;
                        triggerBloodSplatter(targetEnemyX, targetEnemyY, targetEnemyZ);
                        isAnimating = false;
                        setTimeout(() => window.parent.location.reload(), 800); // Seamless reload after animation completes
                    }}
                    ...
                    if (fireballMesh) {{
                        ...
                        fireballMesh.position.y = (pY + 0.5) + (targetEnemyY - (pY + 0.5)) * magicStep + Math.sin(magicStep * Math.PI) * 1.2;
                    }}
                    if (magicStep >= 1.0) {{
                        if (fireballMesh) {{
                        ...
                        triggerBloodSplatter(targetEnemyX, targetEnemyY, targetEnemyZ);
                        isAnimating = false;
                        setTimeout(() => window.parent.location.reload(), 800); // Seamless reload after explosion completes
                    }}
                }}
            }} else {{
            ...
            if (bloodParticles) {{
                ...
                bloodParticles.material.opacity -= 0.01;
            }}
    ...
    with mc2:
        if st.button("⬆️ North", use_container_width=True):
            st.session_state.player_x = max(0, min(48, st.session_state.player_x))
    ...
    with mc3:
        if st.button("⬅️ West", use_container_width=True):
            ...
            st.session_state.player_z = max(0, min(48, st.session_state.player_z))
            process_turn_advance("Exploration Movement")
            st.rerun()
    with mc4:
