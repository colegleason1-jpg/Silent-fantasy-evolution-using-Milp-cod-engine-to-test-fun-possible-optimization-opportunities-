...
with col1:
    ...
    <div style="background: #05050a; border: 1px solid #2a2a40; border-radius: 6px; padding: 4px; text-align: center;">
        ...
        <div style="margin-top: 8px; display: flex; justify-content: center; gap: 8px;">
            ...
            <button id="btn-rest" style="background:#2255cc; color:#fff; border:none; padding:6px 12px; border-radius:4px; cursor:pointer; font-weight:bold; font-size:11px;">⛺ Rest Loop</button>
        </div>
    ...
    <script type="module">
        ...
        // Find the absolute NEAREST enemy character on screen
        let targetEnemyX = pX;
        ...
        function animate() {{
            ...
            if (isAnimating) {{
                if (animType === 'attack') {{
                    animStep += 0.05;
                    // Lunge towards the dynamic nearest enemy coordinate vector
                    ...
                    if (animStep <= Math.PI) {{
                        ...
                        isAnimating = false;
                        setTimeout(() => window.parent.location.reload(), 1200); // Extended pause loop so animations finish nicely
                    }}
                }} else if (animType === 'magic') {{
                    magicStep += 0.025; // Smoother, longer travel trajectory loop
                    if (fireballMesh) {{
                    ...
                    if (magicStep >= 1.0) {{
                        ...
                        triggerBloodSplatter(targetEnemyX, targetEnemyY, targetEnemyZ);
                        isAnimating = false;
                        setTimeout(() => window.parent.location.reload(), 1200); // Extended pause loop for impact & blood effects
                    }}
                }}
            }} else {{
