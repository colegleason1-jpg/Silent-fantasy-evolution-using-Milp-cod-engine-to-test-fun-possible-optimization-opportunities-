...
def generate_world(dynamic_risk_offset=0.0):    
    ...
    creatures = []    
    species_map = {    
        'DR': {'name': 'Dragon', 'biome': 'mountain', 'speed': 8, 'strength': 10, 'magic': 9, 'stealth': 3, 'hp': 3},    
        'GO': {'name': 'Goblin', 'biome': 'swamp', 'speed': 4, 'strength': 3, 'magic': 2, 'stealth': 7, 'hp': 1},    
        'EL': {'name': 'Elf', 'biome': 'forest', 'speed': 6, 'strength': 4, 'magic': 8, 'stealth': 6, 'hp': 2},    
        'WO': {'name': 'Wolf', 'biome': 'plains', 'speed': 7, 'strength': 5, 'magic': 1, 'stealth': 8, 'hp': 2},    
        'BE': {'name': 'Bear', 'biome': 'forest', 'speed': 3, 'strength': 8, 'magic': 1, 'stealth': 4, 'hp': 3},    
        'FA': {'name': 'Fairy', 'biome': 'forest', 'speed': 9, 'strength': 1, 'magic': 10, 'stealth': 9, 'hp': 1}    
    }    
    ...
    starter_offsets = [(2, 0), (-2, 2), (0, -2), (2, 2)]
    starter_types = ['Goblin', 'Wolf', 'Elf', 'Goblin']
    for idx, (ox, oz) in enumerate(starter_offsets):
        ...
        creatures.append({
            ...
            'element': 'fire', 'hp': 1, 'max_hp': 1, 'alive': True,
            'scale': 0.5, 'type': 'enemy'
        })
                    
    devil_biome = random.choice([b for b in biomes if b['biome'] == 'mountain' or b['biome'] == 'forest'])    
    creatures.append({    
        'name': 'The Devil (NPC)', 'x': devil_biome['x'], 'z': devil_biome['z'], 'height': devil_biome['height'],
...
def init_game_state():
    if 'initialized' not in st.session_state:
        ...
        st.session_state.message = "Immersive 3D Fantasy Realm loaded successfully! Starter enemies are nearby to help you begin Quest 1."
        st.session_state.faction_standing = {'Dragons': -10, 'Elves': 20, 'Goblins': -5, 'Fairies': 30}
        st.session_state.turn_log = []
        
        # Persist world and creature states
        world = generate_world()
        st.session_state.world_data = world
        st.session_state.creatures = world['creatures']
        
        # Quest system parameters
        st.session_state.active_quest = "Defeat the starter enemies nearby to complete Chapter 1."
...
def check_quest_progress():
    if st.session_state.kills >= 3 and st.session_state.quest_stage == 1:
        ...
        st.session_state.message = "🌟 Chapter 2 Unlocked: Rumors spread of an awakening darkness in the mountains..."
    elif st.session_state.kills >= 7 and st.session_state.quest_stage == 2:
        st.session_state.quest_stage = 3
        st.session_state.message = "👑 Chapter 3 Unlocked: Factions unite under your banner. The final trials approach!"
        
    if st.session_state.quest_target_kills > 0 and st.session_state.kills >= st.session_state.quest_target_kills:
        st.session_state.message = "🌟 Quest Complete! You earned 100 Gold!"
        st.session_state.gold += 100
        st.session_state.active_quest = "Return to the NPC for your next trial."
        st.session_state.quest_target_kills = 0
...
# ============================================================ 
# STREAMLIT USER INTERFACE LAYOUT (ZERO-SCROLL ABOVE-THE-FOLD)
# ============================================================ 

st.markdown("### 🐉 Dragon Realm: 3D Tactical Campaign")
...
with col1:
    ...
    three_html = f"""
    <div style="background: #05050a; border: 1px solid #2a2a40; border-radius: 6px; padding: 4px; text-align: center;">
        <div id="canvas-container" style="width: 100%; height: 260px; border-radius: 4px; overflow: hidden; box-shadow: inset 0 0 20px rgba(0,0,0,0.9);"></div>
        <div style="margin-top: 8px; display: flex; justify-content: center; gap: 8px;">
            <button id="btn-attack" style="background:#cc2222; color:#fff; border:none; padding:6px 12px; border-radius:4px; cursor:pointer; font-weight:bold; font-size:11px;">⚔️ Attack (Nearest)</button>
            <button id="btn-magic" style="background:#ff8800; color:#fff; border:none; padding:6px 12px; border-radius:4px; cursor:pointer; font-weight:bold; font-size:11px;">🔥 Fireball (Homing)</button>
            <button id="btn-rest" style="background:#2255cc; color:#fff; border:none; padding:6px 12px; border-radius:4px; cursor:pointer; font-weight:bold; font-size:11px;">⛺ Rest (15G)</button>
        </div>
        <p style="color: #00ffcc; font-size: 10px; margin: 4px 0 0 0; font-family: monospace;">
            CHAPTER: {st.session_state.quest_stage} | POS X:{st.session_state.player_x} | Z:{st.session_state.player_z} | KILLS: {st.session_state.kills}
        </p>
    </div>
    <script type="module">
        ...
        creaturesData.forEach((c) => {{
            ...
            if (c.type === 'npc') {{
                const npcMat = new THREE.MeshStandardMaterial({{ color: 0xaa00ff, roughness: 0.2, emissive: 0x440066 }});
                const npcMesh = new THREE.Mesh(new THREE.SphereGeometry(0.5, 12, 12), npcMat);
                npcMesh.position.y = 0.5;
                group.add(npcMesh);
            }} else if (c.name === 'Dragon') {{
        ...
        function triggerEnhancedBloodSplatter(x, y, z) {{
            ...
            const bloodMat = new THREE.PointsMaterial({{ color: 0xcc0000, size: 0.3, transparent: true, opacity: 1.0 }});
        ...
        document.getElementById('btn-magic').addEventListener('click', () => {{
            ...
            const fbMat = new THREE.MeshBasicMaterial({{ color: 0xff7700 }});
                ...
                if (animType === 'attack') {{
                    ...
                    if (targetEnemyGroup) {{
                        targetEnemyGroup.traverse(child => {{
                            if (child.material) {{
                                child.material.color.setHex(Math.sin(dieStep * 10) > 0 ? 0xffffff : 0xcc3333);
                            }}
                        }});
                    }}
                    if (dieStep >= Math.PI) {{
    ...
    """
    components.html(three_html, height=310)

    mc1, mc2, mc3, mc4, mc5 = st.columns(5)
    with mc2:
        if st.button("⬆️ North", use_container_width=True):
            st.session_state.player_z = max(0, min(48, st.session_state.player_z - 2))
            process_turn_advance("Exploration Movement (North)")
            st.rerun()
    with mc3:
        if st.button("⬅️ West", use_container_width=True):
            st.session_state.player_x = max(0, min(48, st.session_state.player_x - 2))
            process_turn_advance("Exploration Movement (West)")
            st.rerun()
    with mc4:
        if st.button("➡️ East", use_container_width=True):
            st.session_state.player_x = max(0, min(48, st.session_state.player_x + 2))
            process_turn_advance("Exploration Movement (East)")
            st.rerun()
    with mc5:
        if st.button("⬇️ South", use_container_width=True):
            st.session_state.player_z = max(0, min(48, st.session_state.player_z + 2))
            process_turn_advance("Exploration Movement (South)")
            st.rerun()
            
    ac1, ac2 = st.columns(2)
    with ac1:
        if st.button("⚔️ Strike (Nearest)", use_container_width=True):
            alive_enemies = [c for c in st.session_state.creatures if c['alive'] and c['type'] == 'enemy']
            if alive_enemies:
                alive_enemies.sort(key=lambda c: math.hypot(c['x'] - st.session_state.player_x, c['z'] - st.session_state.player_z))
                ...
                if dist <= 12:
                    ...
                    if target['hp'] <= 0:
                        ...
                        msg = f"Defeated a {target['name']}! Chain-targeting next alive enemy."
                    else:
                        msg = f"Hit {target['name']}! ({target['hp']}/{target['max_hp']} HP remaining)"
                    process_turn_advance(msg, gold_delta=15, faction_updates={'Dragons': -3})
                else:
                    process_turn_advance("Target is too far away to strike! Move closer.", gold_delta=0)
            else:
                process_turn_advance("No enemies in sight to attack.", gold_delta=0)
            st.rerun()
    with ac2:
        if st.button("🗣️ Talk (NPC)", use_container_width=True):
            npcs = [c for c in st.session_state.creatures if c['type'] == 'npc']
            if npcs:
                ...
                if dist <= 10:
                    st.session_state.active_quest = "The Devil's Request: Slay 4 enemies in the wild."
                    st.session_state.quest_target_kills = st.session_state.kills + 4
                    process_turn_advance("Talked to the Devil. Received a new quest!")
                else:
                    st.session_state.message = "You are not close enough to an NPC to talk."
            st.rerun()

with col2:
    st.subheader("Status & Quests")
    st.markdown(f"**HP:** {st.session_state.health}/{st.session_state.max_health} | **Gold:** {st.session_state.gold}G")
    ...
    if stage == 1:
        st.markdown("**Chapter 1: Vanguard**")
        st.write(f"* Objective: Defeat enemies across the realm.")
        st.write(f"* Progress: {st.session_state.kills} / 3 Kills")
        st.progress(min(1.0, st.session_state.kills / 3))
    elif stage == 2:
        st.markdown("**Chapter 2: Mountain Shadows**")
        st.write(f"* Objective: Hunt down tougher regional threats.")
        st.write(f"* Progress: {st.session_state.kills} / 7 Kills")
        st.progress(min(1.0, st.session_state.kills / 7))
    else:
        st.markdown("**Chapter 3: Realm Unification**")
        st.write(f"* Objective: Secure ultimate dominance and unite factions.")
        st.write(f"👑 **Questline Completed!**")
        st.progress(1.0)
    ...
    if st.session_state.quest_target_kills > 0:
        ...
        st.write(f"**Targets Left to Hunt:** {max(0, remaining)}")
    st.markdown("---")
    
    st.markdown("### Nearby Scanner")
    closest_enemy = min([c for c in st.session_state.creatures if c['alive'] and c['type'] == 'enemy'], 
                        key=lambda c: math.hypot(c['x'] - st.session_state.player_x, c['z'] - st.session_state.player_z), default=None)
    
    if closest_enemy:
        dist = math.hypot(closest_enemy['x'] - st.session_state.player_x, closest_enemy['z'] - st.session_state.player_z)
        status = "🔴 In Range" if dist <= 12 else "⚪ Out of Range"
        st.write(f"**Next Target:** {closest_enemy['name']} ({closest_enemy['hp']} HP)")
        st.write(f"**Distance:** {int(dist)} units {status}")

    st.markdown("### Faction Standing")
    ...
    st.markdown(f"**SCRCAE NPV:** ${opt['objective']:,.0f}")
