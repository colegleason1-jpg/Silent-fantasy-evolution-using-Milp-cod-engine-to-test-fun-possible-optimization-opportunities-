...
def generate_world(dynamic_risk_offset=0.0):    
    # Mythic & Biblical Entity tiers modeled through interventions
...
with col1:
    ...
    three_html = f"""
    <div style="background: #05050a; border: 1px solid #3a1a1a; border-radius: 6px; padding: 4px; text-align: center;">
...
with col2:
    ...
    if closest_enemy:
        ...
        status = "🔴 In Strike Range" out_range = "⚪ Distant"
        st.write(f"**Target:** {closest_enemy['name']} ({closest_enemy['hp']} HP)")
        st.write(f"**Range:** {int(dist)} units")
