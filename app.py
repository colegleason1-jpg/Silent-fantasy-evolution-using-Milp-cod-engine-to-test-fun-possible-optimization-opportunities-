...
def apply_damage_to_nearest(amount, action_name, target_id=None):
    # The server is the sole authority for combat. The renderer may request a
    # target by ID, but it may never choose a different target on its own.
    target = None
    if target_id:
        for candidate in get_live_creatures():
            if candidate.get("type") in ("enemy", "boss") and creature_key(candidate) == str(target_id):
                target = candidate
                break
    if target is None:
...
def check_quest_progress():
    ...
    if stage == 1 and starter_kill_count() >= 3:
    ...
    elif stage == 2 and st.session_state.kills >= 7:
...
def consume_iframe_action():
    ...
    if not st.session_state.get("_kg_query_lock"):
        ...
        # Query payloads can include a target ID: attack:<id> / magic:<id>.
        # This removes the old client/server targeting race that could make a
        # visually killed starter appear to come back after movement.
        action, sep, requested_target = str(raw).partition(":")
        if action == "attack":
            apply_damage_to_nearest(1, "Dark Strike", requested_target if sep else None)
        elif action == "magic":
            apply_damage_to_nearest(2, "Hellfire Blast", requested_target if sep else None)
        elif raw == "rest":
...
with col1:
    ...
    # Render from the authoritative roster only. Dead entities are filtered by
    # get_live_creatures(), so a rerun caused by movement can never resurrect
    # one from the original world_data snapshot.
    ...
    server_target = nearest_hostile()
    if server_target is not None:
        server_target_dist = math.hypot(
            server_target["x"] - st.session_state.player_x,
            server_target["z"] - st.session_state.player_z,
        )
        if server_target_dist > 14:
            server_target = None
    server_target_id = creature_key(server_target) if server_target is not None else None
    server_target_json = json.dumps(server_target_id)
    ...
    <script type="module">
        ...
        const serverTargetId = {server_target_json};
        ...
        let targetEnemyId = serverTargetId;
        ...
        creaturesData.forEach((c, idx) => {{
            ...
            if ((c.type === 'enemy' || c.type === 'boss') && c.id === serverTargetId) {{
                targetEnemyX = cx;
                targetEnemyZ = cz;
                targetEnemyY = cy;
        ...
        function commitActionToGameState(action) {{
            ...
            const targetSuffix = (targetEnemyId && (action === 'attack' || action === 'magic'))
                ? ':' + encodeURIComponent(targetEnemyId)
                : '';
            url.searchParams.set('kg_action', action + targetSuffix);
