    ...
    if st.session_state.gold >= 10:
        if magic_target:
            ...
            if magic_target['hp'] <= 0:
                ...
                st.session_state.kills += 1
                st.session_state.message = f"🔥 Fireball slammed directly into {magic_target['name']}, triggering an explosion and blood splatter! Target incinerated. Looted 50 Gold."
                process_turn_advance("Cast Fireball (Kill)", gold_delta=50, faction_updates={'Fairies': 10})
            else:
                st.session_state.message = f"🔥 Fireball bolted directly into {magic_target['name']} for {dmg} ranged damage with impact blood splatter! ({magic_target['hp']} HP left)"
...
with col1:
    ...
    <script type="module">
        ...
        // Find closest enemy for locking animations accurately
        ...
        function animate() {{
            ...
            if (doMagicAnim && fireballMesh) {{
                ...
                fireballMesh.position.y = (pY + 0.5) + (targetEnemyY - (pY + 0.5)) * magicStep;

                if (magicStep >= 1.0) {{
                    scene.remove(fireballMesh);
                    fireballMesh = null;
                    // Trigger impact blood splatter when fireball connects!
                    triggerBloodSplatter(targetEnemyX, targetEnemyY, targetEnemyZ);
                }}
            }}
