...
def init_game_state():
    if 'initialized' not in st.session_state:
        ...
        st.session_state.render_counter = 0  # <--- Added render counter tracking
...
def process_turn_advance(action_name: str, health_delta=0, gold_delta=0, faction_updates=None):
    st.session_state.turn += 1
    st.session_state.render_counter += 1  # <--- Forces complete component re-mounting/re-render on every action/movement
...
with col1:
    ...
    # STRICTLY FILTER OUT ANY ENTITY WHERE alive IS FALSE
    active_creatures_for_render = [c for c in st.session_state.creatures if c.get('alive', True)]
    ...
    """
    # PASS A UNIQUE DYNAMIC KEY USING render_counter TO FORCE COMPONENT REFRESH
    components.html(three_html, height=310, scrolling=False, key=f"three_view_{st.session_state.render_counter}")
