...
def init_game_state():
    if 'initialized' not in st.session_state:
        ...
        st.session_state.trigger_attack_anim = False
...
def execute_attack():
    ...
    st.session_state.trigger_attack_anim = True # Triggers player lunge and blood effects in 3D
    if hit_target:
...
with col1:
    ...
    trigger_anim = "true" if st.session_state.trigger_attack_anim else "false"

    # 3D Engine with Lunge Combat Visuals and Blood Particle Burst FX
    three_html = f"""
    ...
    <script type="module">
        ...
        const doAttackAnim = {trigger_anim};
        let bloodParticles = null;
        let bloodAnimTimer = 0;
        ...
        creaturesData.forEach(c => {{
            ...
            if (!c.alive) {{
                // Dead enemy falls sideways on the ground instead of disappearing
        ...
        // If attack was triggered, generate blood particle burst and lunge toward closest target
        if (doAttackAnim) {{
            // Find closest active enemy to lunge towards
            ...
            const bloodMat = new THREE.PointsMaterial({ color: 0xff0000, size: 0.22, transparent: true, opacity: 0.9 });
        ...
        function animate() {{
            ...
            // Player Attack Lunge Animation Effect
            if (doAttackAnim && animStep < Math.PI) {{
                playerMesh.position.z = pZ - Math.sin(animStep) * 1.2; // Lunge forward and back
                playerMesh.rotation.y += 0.15;
            }} else {{
                playerMesh.rotation.y += 0.03;
                playerMesh.position.y = pY + Math.sin(Date.now() * 0.003) * 0.2;
            }}

            // Animate Blood Splatter Particles exploding outward
            if (bloodParticles) {{
                ...
                for(let i=0; i<vels.length; i++) {{
                    ...
                    vels[i].y -= 0.012; // Gravity pull on blood
                }}
