...
with col1:
    ...
    # Serialize terrain and creatures safely to inject into Three.js
    ...
    # Enhanced Three.js 3D Viewport with Terrain, Biome Colors, and Entities
    three_html = f"""
    ...
    <script type="module">
        ...
        creaturesData.forEach(c => {{
            ...
            let color = 0xff3355; // Enemy
            if (c.type === 'npc') color = 0xffd700; // NPC / Devil
            else if (c.name === 'Dragon') color = 0xff5500;

            const meshGeo = c.type === 'npc' ? new THREE.SphereGeometry(0.6, 8, 8) : new THREE.BoxGeometry(0.8, 0.8, 0.8);
            const meshMat = new THREE.MeshStandardMaterial({{ color: color, roughness: 0.3 }});
            const entityMesh = new THREE.Mesh(meshGeo, meshMat);
            entityMesh.position.set(cx, cy, cz);
