...
def commit_death(target):
    # Death is authoritative and persistent in the roster itself.
    # Keep the historical dead_ids/quest flags, but also physically remove
    # the dead entity from the live creature roster so a later Streamlit
    # rerun can never re-render the corpse from stale world data.
    ...
    # Remove the exact entity by immutable ID from the authoritative roster.
    # Do not regenerate the world or rebuild creatures here.
    roster = st.session_state.get("creatures", [])
    st.session_state.creatures = [
        c for c in roster if creature_key(c) != cid
    ]
