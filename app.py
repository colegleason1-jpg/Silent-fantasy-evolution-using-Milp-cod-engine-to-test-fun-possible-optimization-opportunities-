...
with col1:
    ...
    # Three.js Hardware-Accelerated 3D Fantasy Viewport Component
    three_html = f"""
    <div style="background: #0d0d1a; border: 1px solid #33334d; border-radius: 8px; padding: 10px; text-align: center;">
        <div id="canvas-container" style="width: 100%; height: 220px; border-radius: 4px; overflow: hidden; box-shadow: inset 0 0 15px rgba(0,0,0,0.9);"></div>
        <p style="color: #00ffcc; font-size: 12px; margin-top: 6px; font-family: monospace;">
            3D REALM VIEWPORT: Player X:{st.session_state.player_x} | Z:{st.session_state.player_z}
        </p>
    </div>
    <script type="module">
        ...
        const dirLight = new THREE.DirectionalLight(0x4b9fff, 1.5);
        dirLight.position.set(10, 20, 10);
        scene.add(dirLight);

        // Ground Grid Plane
        const gridHelper = new THREE.GridHelper(50, 25, 0x4b9fff, 0x1a1a2e);
        gridHelper.position.y = -1;
        scene.add(gridHelper);

        // Player 3D Cone Representation
        const playerGeo = new THREE.ConeGeometry(0.8, 2, 6);
        const playerMat = new THREE.MeshStandardMaterial({{ color: 0x00ffcc, roughness: 0.2, metalness: 0.8, emissive: 0x004433 }});
        const playerMesh = new THREE.Mesh(playerGeo, playerMat);
        scene.add(playerMesh);

        // Map python coords to 3D space
        const targetX = ({st.session_state.player_x} - 24) * 0.4;
        const targetZ = ({st.session_state.player_z} - 24) * 0.4;
        playerMesh.position.set(targetX, 0, targetZ);

        camera.position.set(targetX, targetY = 6, targetZ + 8);
        camera.lookAt(targetX, 0, targetZ);

        // Floating magic particles in 3D
        ...
        const particleMat = new THREE.PointsMaterial({{ color: 0xb04bff, size: 0.3 }});
