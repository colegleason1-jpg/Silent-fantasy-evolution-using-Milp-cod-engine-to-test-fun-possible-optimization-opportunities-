...
with col1:
    ...
    # Filter out dead entities so they are completely purged from the render list
    active_creatures_for_render = [c for c in st.session_state.creatures if c['alive']]
    ...
    with ac1:
        if st.button("⚔️ Strike (Nearest)", use_container_width=True):
            alive_enemies = [c for c in st.session_state.creatures if c['alive'] and c['type'] in ['enemy', 'boss']]
            if alive_enemies:
...
with col2:
    ...
    active_targets = [c for c in st.session_state.creatures if c['alive'] and c['type'] in ['enemy', 'boss']]
