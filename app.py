...
# ============================================================ 
# STREAMLIT USER INTERFACE LAYOUT (COMPACT & ABOVE-THE-FOLD)
# ============================================================ 

st.title("🐉 Dragon Realm: Fantasy 3D World")

# Compact primary action buttons right below title/status header for zero scrolling
...
with col1:
    ...
    three_html = f"""
    <div style="background: #05050a; border: 1px solid #2a2a40; border-radius: 8px; padding: 6px; text-align: center;">
        <div id="canvas-container" style="width: 100%; height: 260px; border-radius: 6px; overflow: hidden; box-shadow: inset 0 0 25px rgba(0,0,0,0.9);"></div>
        <p style="color: #00ffcc; font-size: 11px; margin: 4px 0 0 0; font-family: monospace;">
            X:{st.session_state.player_x} | Z:{st.session_state.player_z} | Kills: {st.session_state.kills}
        </p>
    </div>
    <script type="module">
    ...
    """
    components.html(three_html, height=300)

    # Tight directional movement pad below viewport
...
with col2:
    ...
    st.markdown(f"**Coords:** X: {st.session_state.player_x}, Z: {st.session_state.player_z}")
    ...
    if current_biome:
        ...
        st.markdown(f"**Magic:** {current_biome['magic_element'].upper()} ({current_biome['magic_strength']*105:.0f}%)")
        if current_biome['poi']:
            st.warning(f"📍 {current_biome['poi']['name']}")
