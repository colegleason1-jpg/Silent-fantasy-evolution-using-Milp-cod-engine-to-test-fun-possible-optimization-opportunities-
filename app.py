...
def apply_damage_to_target(amount, action_name, target_id=None):
    # The server is authoritative.  A renderer may suggest a target ID, but
    # only this function decides whether that exact live entity can be hit.
    if target_id:
        target_id = str(target_id)
        target = next((c for c in get_live_creatures()
                       if c.get("type") in ("enemy", "boss") and creature_key(c) == target_id), None)
    else:
    ...
    # Snapshot the target location for the renderer before the entity is
    # removed from the live roster.  This lets the next iframe show a hit
    # effect without ever keeping the dead entity in the authoritative world.
    target_id = creature_key(target)
    target_x, target_z, target_height = target["x"], target["z"], target.get("height", 0)
...
def apply_damage_to_nearest(amount, action_name):
    return apply_damage_to_target(amount, action_name, None)
...
def consume_iframe_action():
    ...
    if not st.session_state.get("_kg_query_lock"):
        ...
        if raw.startswith("attack"):
            _, _, target_id = raw.partition(":")
            apply_damage_to_target(1, "Dark Strike", target_id or None)
        elif raw.startswith("magic"):
            _, _, target_id = raw.partition(":")
            apply_damage_to_target(2, "Hellfire Blast", target_id or None)
        elif raw == "rest":
...
with col1:
    ...
    <script type="module">
        ...
        document.getElementById('btn-attack').addEventListener('click', () => {{
            ...
            // IMPORTANT: commit immediately.  The old implementation waited
            // for the visual animation to finish.  A movement button outside
            // this iframe could rerun Streamlit during that window and destroy
            // the pending action, making a visually dead enemy reappear.
            if (!hasValidTarget) {{
