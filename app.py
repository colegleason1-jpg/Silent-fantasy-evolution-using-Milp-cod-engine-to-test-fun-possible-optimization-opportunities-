...
def rebuild_active_enemies():
    creatures = st.session_state.get('creatures', [])
    assign_enemy_ids(creatures)
    st.session_state.active_enemies = [c for c in creatures if is_hostile(c) and c.get('hp', 0) > 0]

def sync_world_creatures():
    active_ids = {c['id'] for c in st.session_state.get('active_enemies', []) if 'id' in c}
    synced = []
    for creature in st.session_state.get('creatures', []):
        if not is_hostile(creature):
            synced.append(creature)
        elif creature.get('id') in active_ids and creature.get('hp', 0) > 0:
            synced.append(creature)
    st.session_state.creatures = synced
    if 'world_data' in st.session_state:
        st.session_state.world_data['creatures'] = synced

def hard_delete_enemy(enemy_id):
    st.session_state.active_enemies = [e for e in st.session_state.get('active_enemies', []) if e.get('id') != enemy_id]
    st.session_state.creatures = [c for c in st.session_state.get('creatures', []) if c.get('id') != enemy_id]
    if 'world_data' in st.session_state:
...
def strike_nearest_preserved():
    rebuild_active_enemies()
    sync_world_creatures()
    ...
    for creature in st.session_state.creatures:
        if creature.get('id') == target.get('id'):
