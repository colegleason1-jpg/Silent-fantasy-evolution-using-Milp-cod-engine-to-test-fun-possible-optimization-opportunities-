...
def is_creature_alive(creature):
    return creature.get('alive', True) and creature.get('is_alive', creature.get('alive', True)) and creature.get('hp', 0) > 0

def living_creatures(creatures):
    return [c for c in creatures if is_creature_alive(c)]

def living_hostiles(creatures):
    return [c for c in creatures if is_hostile(c) and is_creature_alive(c)]

def assign_creature_uids(creatures):
...
def rebuild_enemy_index():
    assign_creature_uids(st.session_state.get('creatures', []))
    st.session_state.enemies = [c for c in st.session_state.get('creatures', []) if is_hostile(c)]
...
def strike_nearest():
    ...
    if not target:
        st.session_state.message = 'No living enemies remain nearby.'
        return
    if dist > 14:
...
def cast_hellfire():
    hostiles = living_hostiles(st.session_state.get('creatures', []))
    if not hostiles:
        st.session_state.message = 'Hellfire found no living targets.'
        return
    hits = 0
    kills = 0
    ...
    if hits == 0:
        ...
        st.session_state.message = f'Hellfire scorched {hits} enemies and claimed {kills} souls.'
...
with col1:
    ...
    creatures_json = json.dumps([{
        ...
        'alive': c['alive'], 'is_alive': c.get('is_alive', c['alive']), 'type': c['type'], 'hp': c['hp'], 'max_hp': c['max_hp']
    ...
    with mc1:
        if st.button("Move / Advance", use_container_width=True):
            advance_player(dx=2, dz=0, label="Advance Through Wastes")
            st.rerun()
    with mc2:
        if st.button("⬆️ North", use_container_width=True):
            advance_player(dz=-2, label="Crusade Movement (North)")
            st.rerun()
    with mc3:
        if st.button("⬅️ West", use_container_width=True):
            advance_player(dx=-2, label="Crusade Movement (West)")
            st.rerun()
    with mc4:
        if st.button("➡️ East", use_container_width=True):
            advance_player(dx=2, label="Crusade Movement (East)")
            st.rerun()
    with mc5:
        if st.button("⬇️ South", use_container_width=True):
            advance_player(dz=2, label="Crusade Movement (South)")
            st.rerun()
