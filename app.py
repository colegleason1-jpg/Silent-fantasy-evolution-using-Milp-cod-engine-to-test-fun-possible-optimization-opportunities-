...
def generate_world(dynamic_risk_offset=0.0):    
    ...
    creatures = []    
    species_map = {    
        'DR': {'name': 'Dragon', 'biome': 'mountain', 'speed': 8, 'strength': 10, 'magic': 9, 'stealth': 3, 'hp': 3}, # 3 hits to kill
        'GO': {'name': 'Goblin', 'biome': 'swamp', 'speed': 4, 'strength': 3, 'magic': 2, 'stealth': 7, 'hp': 1},    # 1 hit to kill
        'EL': {'name': 'Elf', 'biome': 'forest', 'speed': 6, 'strength': 4, 'magic': 8, 'stealth': 6, 'hp': 2},    # 2 hits to kill
        'WO': {'name': 'Wolf', 'biome': 'plains', 'speed': 7, 'strength': 5, 'magic': 1, 'stealth': 8, 'hp': 2},    # 2 hits to kill
        'BE': {'name': 'Bear', 'biome': 'forest', 'speed': 3, 'strength': 8, 'magic': 1, 'stealth': 4, 'hp': 3},    # 3 hits to kill
        'FA': {'name': 'Fairy', 'biome': 'forest', 'speed': 9, 'strength': 1, 'magic': 10, 'stealth': 9, 'hp': 1}    # 1 hit to kill
    }    
...
with col1:
    ...
    <script type="module">
        ...
        creaturesData.forEach((c) => {{
            ...
            if (c.alive && c.type === 'enemy') {{
                ...
                if (dist < minDist) {{
                    ...
                    // Multi-hit logic check on client side simulation preview
        ...
        function triggerBloodSplatter(x, y, z) {{
            ...
            const bloodMat = new THREE.PointsMaterial({{ color: 0xff0000, size: 0.35, transparent: true, opacity: 1.0 }});
        ...
        function animate() {{
            ...
            if (isAnimating) {{
                if (animType === 'attack') {{
                    ...
                    // Multi-hit flash/hurt animation without disappearing
                    dieStep += 0.08;
                    if (targetEnemyGroup) {{
            ...
            if (bloodParticles) {{
                ...
                bloodParticles.material.opacity -= 0.008;
            }}
    ...
    with mc1:
        if st.button("⚔️ Strike", use_container_width=True):
            # Find nearest alive enemy and reduce its HP
            ...
            if alive_enemies:
                ...
                process_turn_advance("Struck empty space - no enemies nearby.", gold_delta=0)
            st.rerun()
...
with col2:
    ...
    st.markdown(f"**Chapter:** {st.session_state.quest_stage}")
