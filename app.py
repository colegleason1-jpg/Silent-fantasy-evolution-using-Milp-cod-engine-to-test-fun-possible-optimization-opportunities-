...
def generate_world(dynamic_risk_offset=0.0):    
    ...
    # Baseline risk adjusts based on dynamic game state offset
...
def process_turn_advance(action_name: str, health_delta=0, gold_delta=0, faction_updates=None):
    """Callback triggered on any user action: updates state and re-runs solver optimization."""
    st.session_state.turn += 1
    ...
    # Re-run optimization model dynamically based on new faction standings/gold
...
with col1:
    ...
    # Movement Matrix using Callbacks to ensure strict state serialization
