...
def generate_world(dynamic_risk_offset=0.0):    
    ...
    for node_id, alloc in species_allocations.items():    
        if node_id in species_map:    
            ...
            count = max(2, int(alloc['scale'] * 5))    # Ensure plentiful multiple enemy spawns
    ...
    creatures.append({    
        ...
        'element': 'shadow', 'hp': 10, 'max_hp': 10, 'alive': True,    
        'scale': 1.0, 'type': 'npc',    
        'dialogues': [    
...
def init_game_state():
    if 'initialized' not in st.session_state:
        ...
        st.session_state.message = "Immersive 3D Fantasy Realm loaded successfully! Explore and hunt targets."
...
with col1:
    ...
    <script type="module">
        ...
        creaturesData.forEach((c) => {{
            ...
            // Find nearest ALIVE enemy within tactical range (e.g., 10 units)
        ...
        function triggerEnhancedBloodSplatter(x, y, z) {{
            const pCount = 200; // Richer particle count
            ...
            // Vibrant crimson and dark blood mix
        ...
        function animate() {{
            ...
            if (bloodParticles) {{
                ...
                for (let i = 0; i < vels.length; i++) {{
                    ...
                    vels[i].y -= 0.02; // Gravity pull on blood
                }}
    ...
    with mc5:
        if st.button("⬇️ South", use_container_width=True):
            ...
            st.rerun()
    with mc1:
        if st.button("⚔️ Strike", use_container_width=True):
            # Safe distance check in python backend
            alive_enemies = [c for c in world['creatures'] if c['alive'] and c['type'] == 'enemy']
            if alive_enemies:
...
with col2:
    ...
    if current_biome:
        st.markdown(f"**Biome:** {current_biome['biome'].capitalize()} ({current_biome['magic_element'].upper()})")
