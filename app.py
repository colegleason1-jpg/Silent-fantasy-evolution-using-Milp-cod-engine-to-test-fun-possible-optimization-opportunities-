...
def execute_attack():
    ...
    if hit_target:
        ...
        if hit_target['hp'] <= 0:
            ...
            st.session_state.message = f"⚔️ Vanquished {hit_target['name']} with an extended melee lunge and arterial blood splatter! Looted 40 Gold."
            process_turn_advance(f"Defeated {hit_target['name']}", gold_delta=40, faction_updates={'Dragons': -8})
        else:
            st.session_state.message = f"⚔️ Extended melee lunge struck {hit_target['name']} for {dmg} damage with blood spray! ({hit_target['hp']} HP left)"
    ...
    if st.session_state.gold >= 10:
        if magic_target:
            ...
            if magic_target['hp'] <= 0:
                ...
                st.session_state.kills += 1
                st.session_state.message = f"🔥 Fireball projectile traveled across the field, slamming into {magic_target['name']} with a fiery explosion and blood splatter! Looted 50 Gold."
                process_turn_advance("Cast Fireball (Kill)", gold_delta=50, faction_updates={'Fairies': 10})
            else:
                st.session_state.message = f"🔥 Fireball traveled downrange, striking {magic_target['name']} for {dmg} ranged damage with impact blood splatter! ({magic_target['hp']} HP left)"
...
with col1:
    ...
    <div style="background: #05050a; border: 1px solid #2a2a40; border-radius: 6px; padding: 4px; text-align: center;">
        <div id="canvas-container" style="width: 100%; height: 240px; border-radius: 4px; overflow: hidden; box-shadow: inset 0 0 20px rgba(0,0,0,0.9);"></div>
    ...
    <script type="module">
        ...
        let targetEnemyX = pX, targetEnemyZ = pZ - 3, targetEnemyY = pY;
        ...
        function triggerBloodSplatter(x, y, z) {{
            const pCount = 80;
            ...
            const bloodMat = new THREE.PointsMaterial({{ color: 0xff0000, size: 0.28, transparent: true, opacity: 1.0 }});
        ...
        if (doAttackAnim) {{
            triggerBloodSplatter(targetEnemyX, targetEnemyY, targetEnemyZ);
        }}

        if (doMagicAnim) {{
            const fbGeo = new THREE.SphereGeometry(0.4, 10, 10);
            const fbMat = new THREE.MeshBasicMaterial({{ color: 0xff8800 }});
        ...
        function animate() {{
            ...
            // Extended, deliberate melee lunge animation loop
            if (doAttackAnim) {{
                animStep += 0.05; // Slower, highly visible extended step
            ...
            // Extended, cinematic fireball flight path loop
            if (doMagicAnim && fireballMesh) {{
                magicStep += 0.045; // Deliberate long flight duration across screen
                fireballMesh.position.x = pX + (targetEnemyX - pX) * magicStep;
                fireballMesh.position.z = pZ + (targetEnemyZ - pZ) * magicStep;
                fireballMesh.position.y = (pY + 0.5) + (targetEnemyY - (pY + 0.5)) * magicStep + Math.sin(magicStep * Math.PI) * 0.8; // Arc trajectory
    ...
    """
    components.html(three_html, height=280)
