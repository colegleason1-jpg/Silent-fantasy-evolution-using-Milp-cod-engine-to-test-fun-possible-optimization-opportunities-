...
def ensure_enemy_runtime():
    if 'enemy_uid_counter' not in st.session_state:
        st.session_state.enemy_uid_counter = 1
    if 'active_enemies' not in st.session_state:
        st.session_state.active_enemies = []
    if 'dead_enemy_ids' not in st.session_state:
        st.session_state.dead_enemy_ids = set()
...
def assign_enemy_ids(creatures):
    for creature in creatures:
        if is_hostile(creature) and 'id' not in creature:
            creature['id'] = st.session_state.enemy_uid_counter
            st.session_state.enemy_uid_counter += 1

def filter_dead_from_list(creatures):
    dead_ids = st.session_state.get('dead_enemy_ids', set())
    filtered = []
    for creature in creatures:
        if not is_hostile(creature):
        ...
        if creature.get('hp', 0) <= 0:
            if cid is not None:
                st.session_state.dead_enemy_ids.add(cid)
            continue
        filtered.append(creature)
    return filtered

def sanitize_all_creature_state():
    creatures = st.session_state.get('creatures', [])
    assign_enemy_ids(creatures)
    creatures = filter_dead_from_list(creatures)
    st.session_state.creatures = creatures
    st.session_state.active_enemies = [c for c in creatures if is_hostile(c)]
    if 'world_data' in st.session_state:
        assign_enemy_ids(st.session_state.world_data.get('creatures', []))
        st.session_state.world_data['creatures'] = filter_dead_from_list(st.session_state.world_data.get('creatures', []))

def hard_delete_enemy(enemy_id):
    st.session_state.dead_enemy_ids.add(enemy_id)
    sanitize_all_creature_state()
    st.session_state.quest_progress = st.session_state.get('quest_progress', 0) + 1

def nearest_active_enemy():
    sanitize_all_creature_state()
    enemies = [e for e in st.session_state.get('active_enemies', []) if e.get('hp', 0) > 0]
    if not enemies:
        return None, None
    enemies.sort(key=lambda c: math.hypot(c['x'] - st.session_state.player_x, c['z'] - st.session_state.player_z))
    target = enemies[0]
    dist = math.hypot(target['x'] - st.session_state.player_x, target['z'] - st.session_state.player_z)
    return target, dist

def mirror_enemy_hp(enemy_id, new_hp):
    for bucket_name in ['creatures']:
        for creature in st.session_state.get(bucket_name, []):
            if creature.get('id') == enemy_id:
                creature['hp'] = new_hp
                if new_hp <= 0:
                    creature['alive'] = False
    if 'world_data' in st.session_state:
        for creature in st.session_state.world_data.get('creatures', []):
            if creature.get('id') == enemy_id:
                creature['hp'] = new_hp
                if new_hp <= 0:
                    creature['alive'] = False

def move_player_preserved(dx=0, dz=0, label='Move'):
...
def strike_nearest_preserved():
    sanitize_all_creature_state()
    target, dist = nearest_active_enemy()
    if not target:
        st.session_state.message = 'No living enemy remains to strike.'
        return
    if dist > 14:
        ...
        return
    target['hp'] -= 1
    mirror_enemy_hp(target['id'], target['hp'])
    if target['hp'] <= 0:
        st.session_state.kills += 1
        slain_name = target['name']
        hard_delete_enemy(target['id'])
        st.session_state.message = f"Permanently vanquished {slain_name}! Removed from the battlefield."
    else:
        sanitize_all_creature_state()
        st.session_state.message = f"Damaged {target['name']}! ({target['hp']}/{target['max_hp']} HP left)"
    process_turn_advance('Dark Strike')

def hellfire_preserved():
    sanitize_all_creature_state()
    enemies = list(st.session_state.get('active_enemies', []))
    if not enemies:
        st.session_state.message = 'Hellfire found no targets.'
        return
    hits = 0
    slain = []
    for target in enemies:
        ...
        if dist <= 18:
            ...
            mirror_enemy_hp(target['id'], target['hp'])
            if target['hp'] <= 0:
                slain.append((target['id'], target['name']))
    for enemy_id, _ in slain:
        st.session_state.kills += 1
        hard_delete_enemy(enemy_id)
    sanitize_all_creature_state()
    if hits == 0:
        st.session_state.message = 'Hellfire dissipated before reaching any enemy.'
    else:
        st.session_state.message = f'Hellfire scorched {hits} enemies and destroyed {len(slain)} of them.'
...
with col1:
    ...
    sanitize_all_creature_state()
    active_creatures_for_render = list(st.session_state.creatures)
    ...
    with mc1:
        if st.button("Move / Advance", use_container_width=True):
            move_player_preserved(dx=2, dz=0, label="Advance Through Wastes")
            st.rerun()
    with mc2:
        if st.button("⬆️ North", use_container_width=True):
            move_player_preserved(dz=-2, label="Crusade Movement (North)")
            st.rerun()
    with mc3:
        if st.button("⬅️ West", use_container_width=True):
            move_player_preserved(dx=-2, label="Crusade Movement (West)")
            st.rerun()
    with mc4:
        if st.button("➡️ East", use_container_width=True):
            move_player_preserved(dx=2, label="Crusade Movement (East)")
            st.rerun()
    with mc5:
        if st.button("⬇️ South", use_container_width=True):
            move_player_preserved(dz=2, label="Crusade Movement (South)")
            st.rerun()

    ac1, ac2 = st.columns(2)
    with ac1:
        if st.button("⚔️ Strike (Nearest)", use_container_width=True):
            strike_nearest_preserved()
            st.rerun()
    with ac2:
